...
function addEventBuilderViaPopup(form, toStreamId, redirectPage) {
    ...
    if (toStreamNameAndVersion.length != 2) {
        CARBON.showErrorDialog("사용가능한 스트림 ID가 없습니다. 확인해 주십시오.");
        return;
    }
    ...
    if (!reWhiteSpace.test(eventBuilderName)) {
        CARBON.showErrorDialog("이벤트 빌더 명에 사용할 수 없는 문자가 있습니다.");
        return;
    }
    ...
    if (isFieldEmpty) {
        ...
        if ((metaData == "" && correlationData == "" && payloadData == "" && customMappingValue == "enable") || correlationData == "invalid" || payloadData == "invalid" || metaData == "invalid") {
            CARBON.showErrorDialog("매핑 파라미터는 공백일 수 없습니다.");
            return;
        } else {
            new Ajax.Request('../hrta_eventbuilder/hrta_add_eventbuilder_ajaxprocessor.jsp', {
                method:'POST',
                asynchronous:false,
                ...
                onSuccess:function (event) {
                    if ("true" == event.responseText.trim()) {
                        CARBON.showInfoDialog("이벤트 빌더가 추가되었습니다!!", function () {
                            if (redirectPage != "none") {
                                window.location.href = "../hrta_eventstream/hrta_stream_in_flows.jsp?ordinal=1&eventStreamWithVersion=" + toStreamId;
                            }
                        }, null);
                        customCarbonWindowClose();
                    } else {
                        CARBON.showErrorDialog("이벤트 빌더 추가 실패, Exception: " + event.responseText.trim());
                    }
                }
            })
        }
        ...
        if (textData == "" && customMappingValue == "enable") {
            ...
            new Ajax.Request('../hrta_eventbuilder/hrta_add_eventbuilder_ajaxprocessor.jsp', {
                ...
                onSuccess:function (event) {
                    if ("true" == event.responseText.trim()) {
                        CARBON.showInfoDialog("이벤트 빌더가 추가 되었습니다!!", function () {
                            if (redirectPage != "none") {
                                window.location.href = "../hrta_eventstream/hrta_stream_in_flows.jsp?ordinal=1&eventStreamWithVersion=" + toStreamId;
                            }
                        }, null);
...
//    }
    ...
    newCell3.innerHTML = ' <a class="icon-link" style="background-image:url(../cep-admin/images/delete.gif)" onclick="removeInputProperty(this,\'' + 'map' + '\')">Delete</a>';
...
//    }
    ...
    var newCell2 = newTableRow.insertCell(2);
    newCell2.innerHTML = ' <a class="icon-link" style="background-image:url(../cep-admin/images/delete.gif)" onclick="removeInputProperty(this,\'' + 'xml' + '\')">Delete</a>';
...
function addInputXmlProperty() {
    ...
    // Check for white space
    if (!reWhiteSpace.test(propName.value)) {
        error = "To 항목에 사용할 수 없는 문자가 있습니다.";
    }

    if (xpathExpr.value == "") {
        error = "XPath 항목이 없습니다.\n";
    }
...
//    }
    ...
    newCell4.innerHTML = ' <a class="icon-link" style="background-image:url(../cep-admin/images/delete.gif)" onclick="removeInputProperty(this,\'' + 'map' + '\')">Delete</a>';
...
function addInputRegexDef() {
    ...
    var newCell1 = newTableRow.insertCell(1);
    newCell1.innerHTML = ' <a class="icon-link" style="background-image:url(../cep-admin/images/delete.gif)" onclick="removeRegexInputProperty(this,\'' + 'xml' + '\')">Delete</a>';
...
function addInputTextProperty() {
    ...
    if (regexExpr.value == "") {
        error = "Regular expression 이 없습니다.\n";
    }
    ...
    for (var i = 0; i < tableTBody.rows.length; i++) {
        ...
        if(propName.value == column1) {
            error = propName.value +" 는 이미 정의되었습니다.\n";
            break;
        }
    }
...
function addInputWso2EventProperty() {
    ...
    if (!reWhiteSpace.test(propertyName.value)) {
        error = "Input Attribute 이름 항목에 사용할 수 없는 문자가 있습니다.";
    }

    if (propertyValueOf.value == "") {
        error = "Value Of 항목이 없습니다.\n";
    }
...
function disableBuilderStat(eventBuilderName) {
    jQuery.ajax({
                    type:'POST',
                    url:'../hrta_eventbuilder/hrta_update_property_ajaxprocessor.jsp',
...
function disableBuilderTracing(eventBuilderName) {
    jQuery.ajax({
                    ...
                    error:function (msg) {
                        CARBON.showErrorDialog('추적 비활서화 에러' +
                                               ' ' + eventBuilderName);
                    }
                });
}
...
function createPopupEventBuilderUI(streamNameWithVersion) {
    ...
    new Ajax.Request('../hrta_eventbuilder/hrta_popup_create_event_builder_ajaxprocessor.jsp', {
        ...
        onSuccess:function (data) {
            showCustomEventBuilderPopupDialog(data.responseText, "이벤트 빌더 생성", "80%", "", onSuccessCreateEventBuilder, "90%");
        }
    });
}
