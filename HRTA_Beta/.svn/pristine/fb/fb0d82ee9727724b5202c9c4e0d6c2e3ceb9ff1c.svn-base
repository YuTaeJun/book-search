...
function addOutputWSO2EventProperty(dataType) {
    ...
    newCel3.innerHTML = '<p class="fnc-set"><a href="" class="btn-add"  onclick="removeOutputProperty(this,\'' + dataType + '\')"><i class="fa fa-minus-circle fa-2"></i>삭제</a></p>';
...
function removeOutputProperty(link, format) {
	...
	var rowToRemove = link.parentNode;
    //var propertyToERemove = rowToRemove.cells[0].innerHTML.trim();
    rowToRemove.parentNode.removeChild(rowToRemove);
    CARBON.showInfoDialog("Output 프로퍼티가 성공적으로 삭제되었습니다!");
    return;
}
...
function addOutputMapProperty() {
    ...
    newCel2.innerHTML = '<a href="" class="btn-add" onclick="removeOutputProperty(this,\'' + 'map' + '\')"><i class="fa fa-minus-circle fa-2"></i>삭제</a>';
...
function disableFormatterStat(eventFormatterName) {
    jQuery.ajax({
                    type:'POST',
                    url:'../hrta_eventformatter/hrta_stat_tracing-ajaxprocessor.jsp',
...
function createPopupEventFormatterUI(streamId) {	
    ...
    new Ajax.Request('../hrta_eventformatter/hrta_popup_create_event_formatter_ajaxprocessor.jsp', {
        ...
        onSuccess:function (data) {
            showCustomEventFormatterPopupDialog(data.responseText, "이벤트 포멧터 생성", "80%", "", onSuccessCreateEventFormatter, "90%");
        }
    });
}
...
function addEventFormatterViaPopup(form, streamNameWithVersion, redirectPage) {
    ...
    if (!reWhiteSpace.test(eventFormatterName)) {
        CARBON.showErrorDialog("이벤트 포멧터 이름에 사용하실수 없는 글자가 있습니다.");
        return;
    }
    ...
    if (isFieldEmpty) {
        ...
        if ((metaData == "" && correlationData == "" && payloadData == "") && ((advancedMappingCounter % 2) != 0)) {
            ...
            new Ajax.Request('../hrta_eventformatter/hrta_add_event_formatter_ajaxprocessor.jsp', {
                method:'POST',
                asynchronous:false,
                ...
                onSuccess:function (response) {
                    if ("true" == response.responseText.trim()) {
                        CARBON.showInfoDialog("이벤트 포멧터가 성공적으로 추가되었습니다!", function () {
                            if (redirectPage != "none") {
                                window.location.href = "../hrta_eventstream/hrta_stream_out_flows.jsp?ordinal=1&eventStreamWithVersion=" + streamNameWithVersion;
                            }
                        }, null);
                        customCarbonWindowClose();
                    } else {
                        CARBON.showErrorDialog("이벤트 포멧터 추가에 실패하였습니다, Exception: " + response.responseText.trim());
                    }
                }
            })
        ...
        if (textData == "" && ((advancedMappingCounter % 2) != 0)) {
            CARBON.showErrorDialog("매핑 파라미터는 공란일 수 없습니다.");
            return;
        } else {
            new Ajax.Request('../hrta_eventformatter/hrta_add_event_formatter_ajaxprocessor.jsp', {
                method:'POST',
