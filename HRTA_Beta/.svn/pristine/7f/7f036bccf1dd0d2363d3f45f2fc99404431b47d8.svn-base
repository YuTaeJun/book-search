...
<div id="cols">
	...
	<div class="hgroup">
		<h1>이벤트 Out-Flows(
        <a
                href="../hrta_eventstream/hrta_eventStreamDetails.jsp?ordinal=1&eventStreamWithVersion=<%=eventStreamWithVersion%>"><%=eventStreamWithVersion%>
        </a>) </h1>
    </div>
    <div class=con-area>
		<h2>외부 이벤트 Out-flows</h2>
		<div class="act">
			<jsp:include page="../hrta_eventformatter/hrta_event_formatter_outFlows.jsp"
                                 flush="true">
                 <jsp:param name="eventStreamWithVersion"
                            value="<%=eventStreamWithVersion%>"/>
             </jsp:include>
		</div>		
		<h2>내부 이벤트 Out-flows</h2>
		<div class="act">
