...
<script type="text/javascript">
    ...
    function disableStat(eventAdaptorName) {
        jQuery.ajax({
                        type:'POST',
                        url:'../hrta_output_adaptor/hrta_stat_tracing-ajaxprocessor1.jsp',                        
    ...
    function enableTracing(eventAdaptorName) {
        jQuery.ajax({
                        type:'POST',
                        url:'../hrta_output_adaptor/hrta_stat_tracing-ajaxprocessor.jsp',
...
%>
<script type="text/javascript">CARBON.showInfoDialog('이벤트 어답터 삭제 완료.');</script>
<%
    }

    OutputEventAdaptorManagerAdminServiceStub stub = ToogramOutputEventAdaptorUIUtils.getOutputEventManagerAdminService(config, session, request, response);
    OutputEventAdaptorConfigurationInfoDto[] eventDetailsArray =
    		
    		stub.getAllActiveOutputEventAdaptorConfiguration();
...
<div id="cols">
	...
	<div class="act">
		<h2>아답터 생성</h2>
		<p class="fnc-set">
			<a href="../hrta_output_adaptor/hrta_create_eventAdaptor.jsp?ordinal=1" class="btn-add"><i class="fa fa-plus-circle fa-2"></i>Output Event Adaptor 추가</a>							
		</p>
		<p class="lead-t"><%=totalEventAdaptors%> Active 상태 Output Adaptor
		 <% if (totalNotDeployedEventAdaptors > 0) { %><a
    				href="../hrta_output_adaptor/event_adaptor_files_details.jsp?ordinal=1"><%=totalNotDeployedEventAdaptors%>
					Inactive 상태 Output Adaptor</a><% } else {%><%=totalNotDeployedEventAdaptors%>
					Inactive 상태 Output Adaptor <% } %>
					</p>
		<br/>
        ...
        %>
			...
			%>    
           		<tr class="tableOddRow">
					<th><a href="../hrta_output_adaptor/hrta_event_details.jsp?ordinal=1&eventName=<%=eventDetails.getEventAdaptorName()%>&eventType=<%=eventDetails.getEventAdaptorType()%>"><%=eventDetails.getEventAdaptorName()%>
					</a></th>
	                ...
	                <% } else { %>
						...
						<a class="edit" href="../hrta_output_adaptor/hrta_edit_event_details.jsp?ordinal=1&eventName=<%=eventDetails.getEventAdaptorName()%>">
